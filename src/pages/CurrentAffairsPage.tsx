...
const CurrentAffairsPage: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((it) => [it.title, it.category, it.content].some((v) => v.toLowerCase().includes(q)));
  }, [items, query]);
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filtered.map((it) => (
              <tr key={it._id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{it.category}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{new Date(it.publish_date).toLocaleString()}</td>
